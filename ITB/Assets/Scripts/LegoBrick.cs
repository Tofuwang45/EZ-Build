...
public class LegoBrick : MonoBehaviour
{
    ...
    /// <summary>
    /// Reference to BrickIdentifier for build history tracking
    /// </summary>
    private BrickIdentifier brickIdentifier;

    /// <summary>
    /// List of brick IDs this brick is currently connected to
    /// </summary>
    private List<string> connectedBrickIDs = new List<string>();
    ...
    public List<string> GetConnectedBrickIDs()
    {
        ...
        foreach (var socket in socketSnapPoints)
        {
            ...
            if (connectedIdentifier != null && !ids.Contains(connectedIdentifier.uniqueID))
            {
                ids.Add(connectedIdentifier.uniqueID);
            }
        }
    ...
    /// <summary>
    /// Log this brick placement as a build step in the history manager
    /// </summary>
    private void LogBuildStep()
    {
        ...
        List<string> connectedIDs = GetConnectedBrickIDs();

        // Create build step
        BuildStep step = new BuildStep(
            brickID: brickIdentifier.uniqueID,
            brickName: brickIdentifier.brickName,
            connectedParents: connectedIDs,
